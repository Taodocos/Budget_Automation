...
import { saveData } from "@/app/service/apiSaveExpences";
import { fetchDataBackend } from "@/app/service/apigetExpense";
...
const ExpeGrid = () => {
    ...
    const [gridData, setGridData] = useState<ExpenseData[]>([]);
    ...
    const handleInputChange = (index: number, field: keyof ExpenseData, value: string) => {
    ...
    const handleSave = async (index: number) => {
        ...
        if (rowToSave) {
            try {
                await saveData(rowToSave, userId); // Pass userId here
                            ...
                            <tr key={`${row.expenseId}-${index}`} className="text-sm text-gray-700">
                                ...
                                <td className="border p-2">
                                    {editParentCode === index ? (
                                        <input
                                            type="text"
                                            value={row.totalAmount} // Ensure totalAmount is properly bound
